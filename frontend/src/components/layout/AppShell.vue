...
const visibleNav = computed(() =>
  allNav.filter((item) => {
    if (item.officer && !auth.isOfficer) return false
    if (item.hrManager && !auth.isHRManager) return false
    return true
  })
)
...
const titles = {
  ...
  '/compliance/scheme': 'Pension Compliance',
  '/compliance/pension-review': 'Pension Review',
}
